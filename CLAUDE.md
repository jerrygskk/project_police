...
- 改完檔案**先 compile（`py_compile`）驗證語法**，並**主動自我迭代驗證**：能寫單元測試就寫單元測試、能模擬的邏輯（演算法、SQL round-trip）就模擬跑一輪，依結果自行修正再給他，不要把未驗證的 code 直接丟給他。容器沒有 PySide6、無法截圖實跑 GUI（等同無 puppeteer / 模擬器可截圖），這部分主動告知他、請他上機測
