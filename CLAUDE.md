...
- ⚠️ **權限 gate 是每個新功能的必檢項**：任何牽涉「受限身分不可做」的操作（新增／修改／刪除／排序／還原／設定…），**只靠按鈕 `setEnabled(False)` 不夠**——雙擊、序號/行內編輯、鍵盤 Enter、右鍵、拖拉等**替代觸發路徑會繞過按鈕 enabled**。每加一個這類功能就必須：①在**所有**進入點補權限 guard（現行慣例 `if not self._refEditable(): return` / `AuthManager.instance().is_admin()` 等便捷判斷，勿寫字串比較）②上機**以受限身分（archive／user）逐一驗證每條路徑都繞不過去**。此雷已犯過（序號欄雙擊繞過歸檔管理唯讀），詳見 DEVELOPER.md §3「權限」。
