...
> ⚠️ v1.1.2 起，通用 UI（訊息／確認彈窗、`.ui` 載入、Dialog 按鈕樣式常數）已從 `db_utils` 搬到 **`ui_utils/ui_common.py`**，`db_utils` 回歸純資料層。外部一律走門面 `from ui_utils import …`；`ui_utils` 套件內部模組用相對匯入 `from .ui_common import …`。搬出的符號：`msgInfo`／`msgWarning`／`msgCritical`／`confirmBox`／`loadUi`／`BTN_CONFIRM`／`BTN_DANGER`／`BTN_CANCEL`。
...
- **按鈕／子頁籤示意圖**：說明文字裡的按鈕（如「確認發文」）與子頁籤（如「❐ 刑案陳報」）用**預烤圓角 SVG**（`<img>` 內嵌），因 `QTextBrowser` inline 樣式做不出圓角。SVG 由 `python tools/gen_buttons.py` 依 `BUTTONS`／`TABS` 清單批次產出至 `res/buttons/`（別名 `:/btn/`）與 `res/tabs/`（`:/tab/`），配色比照 `lib/theme.py` 烤進圖；對照表 `ui_utils/button_imgs.py`（label→路徑/寬高，入庫）供 `help_content` 查表。改標籤／配色改該腳本重跑、重編 qrc。⚠️ **新增**按鈕（在 `BUTTONS`／`TABS` 加新 key）時，`gen_buttons.py` 只產 SVG 檔，**不會自動把新檔登記進 `res/resources.qrc`**（qrc 是手維護的明確檔案清單）。故新增按鈕的完整步驟為：①`BUTTONS`／`TABS` 加一行 → ②`python tools/gen_buttons.py` 重跑 → ③**手動在 `res/resources.qrc` 補對應 `<file alias="btn/<key>.svg">buttons/<key>.svg</file>` 一行**（漏這步＝該 `:/btn/` 資源不存在，HELP 內嵌會顯示破圖佔位符）→ ④`pyside6-rcc res/resources.qrc -o res/resources_rc.py` 重編。⚠️ `QTextBrowser` 圖片點陣化解析度有上限（intrinsic 取 2× 已封頂），圖片**不會跟內文一樣銳利**；`font-family` 須用裸字型名（逗號清單會被 QtSvg 當成不存在字型 fallback）。
