...
- 要 Claude **基於專業判斷給建議**。他會反感「見風轉舵」——他說 A 你立刻倒向 A、還拿他的話包裝成你的判斷，這會被點名。有不同意見就誠實講，講完理由讓他決定
...
**版本號**
- 版本號**只改最後一位數字**（目前 `v0.9.0-beta.6`）
- 進位與否他決定，不要自己跳版號

**找得到就別問**
- zip / README / code 裡找得到答案的不要問
...
├── sql.py
├── loading_screen.py
├── data_sync_tool.py    Excel→SQLite 匯入工具（單次使用，獨立打包）
├── init_ref_tables.sql  建表 + 參照表初始資料（含 sort_order 初始值）
├── layouts/             所有 .ui 檔（Layout1~7、main_menu）
├── res/                 圖片 / SVG / qrc（含 __init__.py，可被 import）
...
- 觸發：點預覽表編號欄超連結
- 版面常數：`_LABEL_W=120` / `_FIELD_W=340` / `_MARGIN=40`，總寬 500px
...
> 開發中變更（is_active 擴及部門/案類、sort_order 排序功能、列印改 QPrintPreviewDialog、檔案結構重組）尚未進位，待整批驗證後更新版本號。
