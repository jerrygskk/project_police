...
![主選單](docs/img/01-main-menu.png)

`Windows` ・ `免安裝單一執行檔` ・ 目前版本 **v1.1.1**
...
| 手繕發文簽收表、對著名冊抄 | **簽收單一鍵列印**或存 PDF |
| 在一堆檔案裡翻找某份公文 | **打關鍵字即時搜尋**，不分大小寫 |
| 刪錯了只能重打 | **資源回收筒可還原**，每天自動備份 |
...
| **資料庫設定** | 維護人員／部門／案類、變更密碼、設定歸檔資料夾、跨年度重置、資源回收筒 |
...
雙擊 **`Police-Document-Manager.exe`** 即可使用。第一次開啟建議先做兩件事：

- 進「**資料庫設定 → 人員管理／部門管理**」，把單位的人員與部門建好。
- 進「**資料庫設定 → 歸檔資料夾**」，指定掃描 PDF 要放的資料夾，歸檔功能才會自動帶入。

> 💡 **想先看操作圖解**？下載頁附有 **`Quick_Start.pdf` 速查卡**（兩頁 A4），涵蓋每個功能的重點步驟。

### 登入（管理功能才需要）

一般收發文、建檔、列印**不需登入**即可使用。只有刪除管理、設定維護、操作紀錄等需要以管理身分登入：

- 進「**資料庫設定**」輸入密碼登入。
- 預設密碼——管理者：`admin`、歸檔管理：`0000`。

> ⚠️ **首次登入後請立即至「變更密碼」修改**，預設密碼僅供初次設定使用。

---

## 典型的一天

1. 早上收到上級交辦單 → 「**交辦單收文**」登錄，設好承辦人和限辦日期。
2. 公文辦好要發出 → 「**交辦單發文**」輸入文號發文；要呈報的案件走「**公文陳報**」。
3. 隨時想知道哪些快到期 → 「**資料庫瀏覽**」看顏色、用「逾期未回」一鍵篩出待辦。
4. 發文當天 → 「**簽收單列印**」印出簽收表給收件人簽收。
5. 公文掃描成 PDF → 「**檔案歸檔**」自動比對、一鍵歸檔。
6. 不小心刪錯、或想查誰動過資料 → 「**資料庫設定 → 資源回收筒**」還原、「**操作紀錄**」追查。
...
**Q：要連網路嗎？需要安裝嗎？**
都不用。純單機程式，雙擊即用，所有資料都在本機 `dbfile.db`。

**Q：搜尋區分大小寫嗎？**
不分大小寫，輸入片段即時比對。

**Q：閒置會怎樣？**
管理身分閒置 10 分鐘會自動登出（降回一般使用者）；任何身分閒置 20 分鐘程式會自動關閉，請留意未送出的輸入。
