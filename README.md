...
| **資料庫設定** | 維護人員／部門／案類、變更密碼、設定歸檔資料夾、自訂簽收表標題、跨年度重置、資源回收筒 |
...
- 進「**歸檔資料夾**」指定掃描 PDF 要放的資料夾，歸檔功能才會自動帶入。
...
「檔案歸檔」與「資料庫瀏覽開啟 PDF」都依賴這項設定。到「**資料庫設定 → 歸檔資料夾**」設定，重點是 **一個「本年度資料夾」底下放兩個分類子資料夾**：

<p align="center">
  <img src="docs/img/07-archive-folder.png" alt="設定歸檔資料夾對話框" width="460">
</p>
...
**Q：簽收表上的單位名稱可以改嗎？**
可以。到「資料庫設定 → 簽收表設定」（需管理者登入），即可改三張簽收表的標題與機關名稱。預設機關名為 `○○`，請改成貴單位名稱。未改過時，列印頁上方會出現紅字提醒。
