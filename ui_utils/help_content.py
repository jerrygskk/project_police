...
HELP_PAGES = {
    ...
    6: [
        ...
        ("sec", "系統設定", [
            ("p", "左側「系統設定」子頁集中四個區塊，各區塊修改後須按該區塊的「儲存」才會寫入。"),
            ...
            ("p", _note("簽收表標題") + "（僅管理者）："),
            ("ul", [
                "自訂三張簽收表（交辦／刑案／一般）的標題文字與刑案現行犯的免簽收註記，通常填入本單位名稱與簽收表正式名稱。",
...
QUICKSTART = {
    ...
    6: ("維護人員／部門／案類等基礎資料、系統設定與跨年度重置。本頁供管理者使用；歸檔管理僅能變更密碼與設定歸檔資料夾。",
        ["新增或修改清單項目（雙擊整列可修改，含停用／啟用、別名）。",
         "拖拉整列（或單擊序號欄輸入數字）調整顯示順序，務必按「儲存排序」才會寫入。",
         "「系統設定」子頁設定歸檔資料夾、簽收表標題、閒置逾時與唯讀設定。"],
