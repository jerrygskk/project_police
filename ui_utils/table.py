...
# 固定寬度欄位（格式固定，不需動態量）
# 新增欄位時在此登記即可，autoResizeTable 會自動套用
FIXED_COL_WIDTHS = {
    "交辦單編號": 110,
    "限辦日期":   130,
    "發文日期":   130,
...
def autoResizeTable(table):
    """
    量出欄寬後：
    - 加總 < 可用寬度 → stretch_col 吃剩餘空間（填滿視窗）
    - 加總 > 可用寬度 → 保持原寬，scrollbar 自動出現
    """
...
def setupPreviewTable(table, headers):
    ...
    # 決定 stretch 欄：有刪除欄(col0空白)時 stretch=2，否則 stretch=1
